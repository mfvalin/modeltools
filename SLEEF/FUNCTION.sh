...
# ${Vlibm} function calling libm (only done if precision <= 15)
((PrEcIsIoN <= 15)) && cat <<EOT

void ${Vlibm}${FuNcTiOn}_f(float *f, float *r, int n){
  int i;
  for(i=0 ; i<n ; i++) r[i] = ${FuNcTiOn}f(f[i]);
}

void ${Vlibm}${FuNcTiOn}_d(double *f, double *r, int n){
  int i;
...
void ${Vlibsleef}${FuNcTiOn}_f${PoStFiX}(float *f, float *r, int n){
  ...
  while(i<n){
    r[i] = Sleef_cinz_${FuNcTiOn}f1_u${PrEcIsIoN}purec(f[i]) ;
    i++;
  }
#endif
#endif
#else
  ${Vlibm}${FuNcTiOn}_f(f, r, n);
#endif
}
