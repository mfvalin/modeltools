...
* */
        SUBROUTINE RPN_COMM_transpose_44(
     %             za,min1,max1,n1g,n2,min3,max3,n3g,zb,direction)
        use rpn_comm
        implicit none
...
!
        if (direction == RPN_COMM_FORWARD_X .or. 
     %     direction == RPN_COMM_BACKWARD_X ) then
          npe=pe_nx
          pecomm=pe_myrow
          mype=pe_mex
        else if (direction == RPN_COMM_FORWARD_Y .or. 
     %          direction == RPN_COMM_BACKWARD_Y ) then
          npe=pe_ny
          pecomm=pe_mycol
          mype=pe_mey
        else
        ...
        if (npe == 1) then  ! only one PE in this direction
          call transpose_44
     %         (za,min1,max1,zb,n1g,n2*(max3-min3+1),128,direction)
        else
          allocate(ndim(2,npe))
          allocate(scnts(npe),sdispls(npe),rcnts(npe),rdispls(npe))
          call MPI_allgather( (/ n2,max3-min3+1 /), 2, MPI_INTEGER,
     %                         ndim,2,MPI_INTEGER,pecomm,ierr)
          n1p = (n1g+npe-1)/npe
          n3p = (n3g+npe-1)/npe
          scnts = n3p
...
!       backward direction : transpose b into a
        ...
        !
        if (direction == RPN_COMM_FORWARD_X .or. 
     %      direction == RPN_COMM_FORWARD_Y ) then
          do i0=1,ni,block
