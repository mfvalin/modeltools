...
! SYNOPSIS
module convip_plus_int
  implicit none
  logical, save :: verbose = .true.
end module convip_plus_int

SUBROUTINE CONVIP_plus_set_opt(option, value)
  use convip_plus_int
  implicit none
  character(len=*), intent(IN) :: option
  integer, intent(IN) :: value
  if(trim(option) == 'msglevel') verbose = value > 0
end SUBROUTINE CONVIP_plus_set_opt
...
!
      if(NEWENCODING)then    ! new style excoding
          ...
          if (p .lt. low_val(kind) .or. p .gt. hi_val(kind)) then
              if(verbose) write(6,6006) p,low_val(kind),hi_val(kind)
              ip = -999999
              return
          endif
          iexp = 4
