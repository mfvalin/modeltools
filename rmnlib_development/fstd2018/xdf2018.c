 ...
 *  Revised to add large file and large record capability (M.Valin, 2018)    *
 *                                                                           *
 *                                                                           *
 *****************************************************************************/
#include <stdio.h>
...
static INT_32 add_dir_page(int file_index,int wflag)
{
   ...
   if(f->npages >= MAX_DIR_PAGES) {
      ...
      return(error_msg("add_dir_page",ERR_DIR_FULL,ERROR));
   }
   page_size = sizeof(full_dir_page) +
                8*(f->primary_len)*ENTRIES_PER_PAGE;

// ADD CHECK HERE that f->nxtadr is still below (2G - page_size/4)  (nxtadr is in 32 bit WORD units)
// we do not want to add fudged address coding to the directory page address chain
...
static INT_32 add_dir_pages(int file_index,int wflag, int npages) // add several pages
{
  int status;
  int i;
  register file_table_entry *f;

  /* check if file exists (also done by add_dir_page, but we want to avoid spurious error messages) */
  if( (f = file_table[file_index]) == NULL) return (ERR_NO_FILE);

  /* check if we can add a directory page, and if memory is available (also done by add_dir_page, but we want to avoid spurious error messages) */
  if(f->npages >= MAX_DIR_PAGES) {
    sprintf(errmsg,"Too many records, no more directory pages available");
  ...
  return (i == 0);  // were we able to add at least one page ?
}

