...
/* same length, almost same layout as buffer_interface, but ... */
/* for now this structure is only used by large record / huge file standard files */
/* understood by c_xdf_64_get and c_xdf_64_put */
/* nwords and nbits from interface become nwords64 in buffer_interface_64 */
/* buf78 union from interface becomes nbits64 in buffer_interface_64 */
/* buf9 expected to be -1 to flag a long record */
/* upper 24 bits of nwords64, upper 16 bits of nbits64 expected to be 0 */
typedef struct{
        uint64_t nwords64;      /* dimension of buffer (in words) (40 bits) */
        uint32_t data_index;    /* starting position of data (after the keys) */
...
typedef buffer_interface_64 *buffer_interface_64_ptr;
